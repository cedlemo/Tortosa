...
parser = Rtruckboris::HeaderParser.new(gtk_window, header_paths)
parser.parse(true)
...
def rename(name)
  "static VALUE rtortosa_#{name.gsub("gtk_window_","")}#{O_BRACKET}VALUE self"
end

def build_c_params(params)
  buf=[]
  params.each do |p|
    if p.getType.getName == 'GtkWindow *'
      buf << 'GTK_WINDOW(backbone.window.widget)'
    else
      buf << "r_#{p.getName}"
    end
  end
...
def get_callback_parameters_number(params)
  if params.any? {|p| p.getType.getName == 'GtkWindow *'}
    params.size - 1
  else
    params.size
  end
...
def build_ruby_wrapper(f)
  ...
  s = rename(f.getName)
  params.each do |p|
    s += ", VALUE #{p.getName}" unless p.getType.getName =~ /gint\s\*|GtkWindow\s\*/
  end
...
def to_be_rejected_on_params(params, f)
  ret = false
  params.each do |p|
    if !p.getType.getName.match(/((const)*\s*gint)|((const)*\s*gchar)|(gboolean)|(void)|(GtkWindow\s+\*)/)
      ret = true
...
def write_gtkwindow_wrapper(functions)
  s = 'void gtk_window_wrapper(VALUE module)' + O_CURLY_BRACKET + NEWLINE
  functions.each do |f|
    s += %Q{  rb_define_module_function(module, 
                                        "#{f.getName.gsub('gtk_window_','')}", 
                                        rtortosa_#{f.getName.gsub('gtk_window_','')},
                                        #{function_is_void_getter(f) ? 0 : get_callback_parameters_number(f.getParameters)});} + NEWLINE
  end
  s += C_CURLY_BRACKET
end
wrapper_c = File.new('../gtk_window_methods.c', 'w')
wrapper_h = File.new('../gtk_window_methods.h', 'w')

wrapper_h.puts('#ifndef GTK_WINDOW_METHODS_H')
wrapper_h.puts('#define GTK_WINDOW_METHODS_H')
wrapper_h.puts('#include <ruby.h>')
wrapper_h.puts('#include <gtk/gtk.h>')
wrapper_h.puts('#include "backbone.h"')
wrapper_h.puts('void gtk_window_wrapper(VALUE);')
