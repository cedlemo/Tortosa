...
source = '/usr/include/gtk-3.0/gtk/gtkwindow.h'
...
filter.add_param_to_reject('GtkWindowPosition')
#reject deprecated
filter.add_name_to_reject('gtk_window_set_transient_for')
filter.add_name_to_reject('gtk_window_set_opacity')
filter.add_name_to_reject('gtk_window_get_opacity')
filter.add_name_to_reject('gtk_window_reshow_with_initial_size')
filter.add_name_to_reject('gtk_window_set_has_resize_grip')
filter.add_name_to_reject('gtk_window_get_has_resize_grip')
filter.add_name_to_reject('gtk_window_resize_grip_is_visible')
...
fq.is_getter_by_return_instructions do |function|
  ...
  function.getParameters.each do |parameter|
    ...
    if ptype.match(/\*/) && !ptype.match(/(PangoFontDescription)|(GtkNotebook)|(VteTerminal)|(char)|(GdkRGBA)|(GtkWidget)|(GtkWindow)/)
      test = false
    end
  end
  test
...
wrapper.wrapper_c_arguments_instructions do |parameter|
  if parameter.getType.getName == 'GtkWindow *'
    'window'
  else
...
wrapper.wrapper_r_return_instructions do |function|
  ...
  if fq.is_getter_by_return(function)
    ...
    function.getParameters.each do |p|
      if !p.getType.getName.match(/GtkWindow/)
...
out = Wrapper::OutputFiles.new('../gtk_window_methods')
out._h.puts(File.open('gtk_window_methods_h', 'rb') { |f| f.read })
out._c.puts(File.open('gtk_window_methods_c_1', 'rb') { |f| f.read })

sorter.functions_to_parse.each do |f|
    out._c.puts(generate_setter_handler(f, wrapper, fq))
end

out._c.puts(File.open('gtk_window_methods_c_2', 'rb') { |f| f.read })
...
sorter.functions_to_parse.each do |f|
  out._c.puts(%{  rb_define_method(c_window,
                                        "#{f.getName.gsub('gtk_window_', '')}",
