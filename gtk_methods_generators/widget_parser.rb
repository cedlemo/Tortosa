...
source = '/usr/include/gtk-3.0/gtk/gtkwidget.h'
...
to_match = ['(const\s)*\s*int\s*\*', '(const\s)*\s*gchar\s*\*', '(g)*boolean',
            'void', '(g)*double', '(g)*long', 'uint16', 'GtkWidget']
...
wrapper.wrapper_c_arguments_instructions do |parameter|
  if parameter.getType.getName == 'GtkWidget *'
    'widget'
  else
...
wrapper.wrapper_r_return_instructions do |function|
  ...
  if fq.is_getter_by_return(function)
    ...
    function.getParameters.each do |p|
      ...
      s += "  rb_ary_push(ret, #{p.getName});" + Wrapper::NEWLINE unless p.getType.getName =~ /GtkWidget/
    end
...
out = Wrapper::OutputFiles.new('../gtk_widget_methods')
out._h.puts(File.open('gtk_widget_methods_h', 'rb') { |f| f.read })
out._c.puts(File.open('gtk_widget_methods_c_1', 'rb') { |f| f.read })
...
sorter.functions_to_parse.each do |f|
  out._c.puts(%{  rb_define_method(c_widget,
                                        "#{f.getName.gsub('gtk_widget_', '')}",
